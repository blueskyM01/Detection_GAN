...
class mx_BE_Generator(keras.Model):
    ...
    def call(self, z):
        ...
        x = self.module1(x)
        x = self.mx_scale(x) # 16x16
        x = self.module2(x)
        x = self.mx_scale(x) # 32x32
        x = self.module3(x)
        x = self.mx_scale(x) # 64x64
        x = self.module4(x) #64x64
        # x = self.mx_scale(x) # 128x128
        # x = self.module5(x)
        # x = self.mx_scale(x) # 256x256
        # x = self.module6(x)
        x = self.to_image(x)
        return x
...
class mx_BE_Discriminator(keras.Model):
    def __init__(self, filter_num):
        ...
        self.fc1 = layers.Dense(8 * 8 * filter_num * 4)
    ...
    def call(self, image):
        ...
        x = tf.reshape(x, [-1, 8, 8, self.filter_num * 4])
        x = self.fc1(x)
